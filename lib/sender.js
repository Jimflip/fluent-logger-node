...
FluentSender.prototype.emit = function(label, data, time, callback)
{
  var self = this;
  var item = self._makePacketItem(label, data, time, callback);
...
FluentSender.prototype.end = function(label, data, callback)
{
  ...
  {
    self.emit(label, data, null, function(err)
    {
      self._close();
  ...
  if( self._socket === null )
  {
    ...
    self._socket.on('error', function(err)
    {
      if( self._socket )
      {
        ...
        self._socket = null;
        self._eventEmitter.emit('errorr', err);
      }
    });
